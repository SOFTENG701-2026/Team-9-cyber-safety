...
export function VisualNovelPlayer({ story, onEnd, className }: VisualNovelPlayerProps) {
  ...
  const sortTriggerSceneIds = useMemo(() => new Set(["m2-10", "m3-19", "m3-41"]), []);
  ...
  useEffect(() => {
    ...
    const isActivity = sortTriggerSceneIds.has(scene.id) || emailTriggerSceneIds.has(scene.id);
    ...
    return () => clearTimeout(t);
  }, [scene, recentNonActivitySceneIds, sortTriggerSceneIds, emailTriggerSceneIds, showHistoryPanel]);
